...
export default function EventDetails() {
  ...
  if (isPending) {
    content = (
      <div className='w-[40rem] mx-auto my-8 bg-[#343b3f] rounded-lg overflow-hidden text-center'>
      ...
      <>
        ...
        <div className='w-[40rem] mx-auto my-8 bg-[#343b3f] rounded-lg overflow-hidden'>
          ...
          <div className='px-12 pb-12'>
            <div>
              <p className='text-lg text-[#b6cad5] font-bold'>{data.location}</p>
              <time
                dateTime={`Todo-DateT$Todo-Time`}
                className='text-2xl my-2 block'>
      ...
      {isDeleting && (
        <Modal onClose={handleStopDelete}>
          ...
          <div className=" flex justify-end items-center gap-8">
            ...
            {!isPendingDeletion && (
              <>
                <button onClick={handleStopDelete} className="font-bold text-[#3f0c26] hover:text-[#7c184c]">
                  Cancel
                </button>
                <button onClick={handleDelete} className="px-6 py-2 bg-[#e30d7c] text-white font-bold rounded shadow hover:bg-[#e30d5b] hover:shadow-lg">
                  Delete
                </button>
              </>
            )}
